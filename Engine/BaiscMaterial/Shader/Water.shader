...
uniform float alpha : hint_range(.0, 1.0) = .4;
uniform float border_distance = .1;
...
void vertex(){
	...
	*/
	...
	uv1_power_normal=pow(abs(NORMAL),vec3(.0));
	uv1_power_normal/=dot(uv1_power_normal,vec3(uv1_blend_sharpness));
...
void fragment()
{
	...
	vec3 water_color = vec3(water(uv * tile, vec3(0,1,0), TIME * distortion_speed));
	vec3 foam_color  = FOAM_COL.rgb;
	vec3 border = vec3(clamp(smoothstep(world_pos.z+border_distance,world_pos.z,VERTEX.z),0.0,1.0));
    ALBEDO = mix(water_color, foam_color.rgb, vec3(border));
